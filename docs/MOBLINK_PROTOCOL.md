...
Пока телефон сообщает тот же tunnel endpoint, sender сохраняет свой исходный локальный UDP socket при SRTLA recovery. Это обязательное поведение: relay запоминает адрес и порт первого UDP-пакета от streamer и возвращает ответы именно туда. Новый локальный socket создаётся только вместе с новым телефонным endpoint, опубликованным control plane.
